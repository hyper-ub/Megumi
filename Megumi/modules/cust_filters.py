...
import telegram
from telegram import ParseMode, InlineKeyboardMarkup, Message, CallbackQuery
...
from Megumi import dispatcher, LOGGER
...
from Megumi.modules.helper_funcs.misc import build_keyboard_parser
from Megumi.modules.helper_funcs.msg_types import get_filter_type
from Megumi.modules.helper_funcs.string_handling import (split_quotes,button_markdown_parser,escape_invalid_curly_brackets,markdown_to_html,)
...
def stopall(update, context):
    ...
    if not usermem.status == "creator" and user.id not in SUDO_USERS:
      ...
      msg.reply_text(f"Are you sure you would like to stop All filters in {chat.title}? This cannot be undone.",
                   reply_markup = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                "Yes I'm sure", callback_data="f_stop")],
            [InlineKeyboardButton(
                "Cancel", callback_data="f_cancel"),
                   ]]))

@run_async     
def stopall_button(update, context):
    ...
    if query_match =="f_stop":
      if usermem.status == "creator" or query.from_user.id in SUDO_USERS:
        ...
        msg.edit_text(f"Successfully cleaned {count} filters in {chat.title}.")
      
      else:
        query.answer("Only chat creator of this group can do this.")
    
    elif query_match == "f_cancel":
      if usermem.status == "creator" or query.from_user.id in SUDO_USERS:
         msg.edit_text("Removing of all filters cancelled")
      else:
...
*Example:* `/filter "hey there" How you doin?`
 ...
 • `/stopall`*:* remove all chat filters at once, this cannot be undone. `(chat creator only)`
...
REMOVEALLFILTER_HANDLER = CommandHandler("stopall", stopall, filters=Filters.group)
STOPALL_CALLBACK_HANDLER = CallbackQueryHandler(stopall_button, pattern=r"f_")
...
dispatcher.add_handler(STOPALL_CALLBACK_HANDLER)
